 "use strict";
    ...
    const progressStars = document.querySelectorAll("#progressStars span");

    const finalModal = document.getElementById("finalModal");
    const finalModalBackdrop =
      document.getElementById("finalModalBackdrop");
    const closeFinalModalButton =
      document.getElementById("closeFinalModalButton");
    ...
    const totalWishes = wishButtons.length;

    let lastFocusedElement = null;
    let showFinalAfterClosingWish = false;
    let finalMessageHasBeenShown = false;
    ...
    function openWish(button) {
      ...
      const wishId =
        ...
        String(Array.from(wishButtons).indexOf(button));

      if (!openedWishes.has(wishId)) {
        openedWishes.add(wishId);
    ...
    /**
     * Обновляет прогресс открытых пожеланий.
     */
    function updateWishesProgress() {
      ...
      const openedCount = openedWishes.size;
      const progressPercent =
        totalWishes === 0
          ? 0
          : (openedCount / totalWishes) * 100;

      progressCounter.textContent =
        `${openedCount} из ${totalWishes}`;
      ...
      progressStars.forEach((star, index) => {
        const isOpened = index < openedCount;

        star.textContent = isOpened ? "★" : "☆";
        star.classList.toggle("is-active", isOpened);
      });

      if (
        totalWishes > 0 &&
        openedCount === totalWishes &&
        !finalMessageHasBeenShown
      ) {
        wishesProgress.classList.add("is-complete");
        showFinalAfterClosingWish = true;
    ...
    /**
     * Показывает финальное поздравление.
     */
    function openFinalMessage() {
      if (!finalModal || finalMessageHasBeenShown) {
        document.body.classList.remove("modal-open");
        return;
      }

      finalMessageHasBeenShown = true;
      showFinalAfterClosingWish = false;
      ...
      window.setTimeout(() => {
        closeFinalModalButton?.focus();
      }, 80);
    }

    /**
     * Закрывает финальное поздравление.
     */
    ...
    closeFinalModalButton?.addEventListener(
      "click",
      closeFinalMessage
    );

    finalModalBackdrop?.addEventListener(
      "click",
      closeFinalMessage
    );
